...
async function getLikes() {
  fetch(
    'https://us-central1-involvement-api.cloudfunctions.net/capstoneApi/apps/8ae878c40e24371ea51406311a0a2161/likes',
...
fetch('https://api.tvmaze.com/shows?page=0')
  ...
  .then(async (data) => {
    ...
    const showLikes = await Promise.all(shows).then((shows) => {
      shows.map((show) => getLikes(show.id));
    });
